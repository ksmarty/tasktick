    ...
    <div className={cn('grid grid-cols-7 gap-1', className)} role="img" aria-label={`This week: ${description}`}>
      {days.map((day) => (
        <div
          ...
          aria-hidden
          className={cn(
            'flex flex-col items-center gap-1 rounded-ios-sm px-0.5 pt-1 pb-1.5',
            day.isToday && 'ring-1 ring-label',
          )}
        >
          <span className={cn('text-caption-2 leading-none', day.due ? 'text-secondary' : 'text-tertiary')}>
            {day.initial}
          </span>
          <span
            className={cn('h-1.5 w-full rounded-full', day.logged === 0 && (day.due ? 'bg-fill-secondary' : 'bg-fill-quaternary'))}
