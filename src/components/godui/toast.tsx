 ...
 * GodUI — Toast.
 *
 * Source as published by the `@godui/toast` registry entry. Kept in
 ...
 *
 * Local changes:
 *  1. `cn()` is imported from `@/lib/utils` and used for the toast item's class
 ...
 * `z-modal`. GodUI's published theme does not ship those tokens.
 *
 ...
 * drop. The public API (`toast`, `ToastProvider`, and all exported types) is
 * unchanged.
 */
...
function ToastItem({
  ...
  return (
    <motion.li
      ...
      className={cn(
        'absolute inset-x-0 flex w-full cursor-grab items-start gap-3 rounded-xl border bg-card p-4 text-card-foreground shadow-lg active:cursor-grabbing',
        isBottom ? 'bottom-0' : 'top-0',
        VARIANT_CLASS[record.variant ?? 'default'],
      )}
    >
  ...
  const portal = portalTarget
    ? createPortal(
        <motion.ol
          ...
          className={cn(
            'fixed z-[60] w-[min(24rem,calc(100vw-2rem))]',
