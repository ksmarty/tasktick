...
/**
 * GodUI — Hold Confirm Button.
 *
 * Source as published by the `@godui/hold-confirm-button` registry entry. Kept in
 ...
 *
 * Local changes:
 *  1. `cn()` is imported from `@/lib/utils` and used for the button's class list
 ...
 *
 * `[--button-radius:…]` / `px-[var(--button-px-md)]` are kept as published: the
 * GodUI button scale is defined in `globals.css`, and a CSS-variable reference is
 * not an arbitrary value.
 *
 * Used for the destructive confirmations (delete a task, delete a selection):
 * a hold is a deliberate act, so the one irreversible button in the app is not a
 * tap away from a mis-touch.
 */
    ...
    return (
      ...
      >
        <motion.span
          ...
          className={cn('absolute inset-0 origin-left', fillClass[variant])}
        />
