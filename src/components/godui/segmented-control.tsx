...
/**
 * GodUI — Segmented Control.
 *
 * Source as published by the `@godui/segmented-control` registry entry. Kept in
 ...
 *
 * Local changes:
 *  1. `cn()` is imported from `@/lib/utils` and used for the container and each
 *     option button instead of string concatenation, so a caller's `className`
 ...
 * drop. The public API (`SegmentedControl`, `SegmentedOption`,
 * `SegmentedControlSize`, `SegmentedControlProps`) is unchanged.
 */
        ...
        {options.map((opt) => {
          ...
          return (
            <button
              ...
              className={cn(
                'relative isolate inline-flex h-full items-center justify-center gap-1.5 rounded-md px-3 font-medium [transition:color_150ms_ease] focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 focus-visible:ring-offset-background active:scale-[0.97] disabled:pointer-events-none disabled:opacity-40',
                active
                  ? 'text-foreground'
