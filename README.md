...
# Required: signs session cookies and encrypts stored CalDAV passwords.
...
| Variable | Notes |
|---|---|
| `BETTER_AUTH_SECRET` | **Required in production.** `openssl rand -base64 32`. Rotating it signs everyone out and invalidates stored CalDAV passwords. |

### Common

| Variable | Default | Notes |
|---|---|---|
| `APP_URL` | `http://localhost:3000` | Public origin. Used for auth callbacks and `webcal://` links. Must match the URL you actually visit. |
...
519 tests across 31 files, with no network and no external services:
